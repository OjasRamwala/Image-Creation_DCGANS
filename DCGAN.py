...
dataloader = torch.utils.data.DataLoader(dataset, batch_size = batchSize, shuffle = True, num_workers = 2) # We use dataLoader to get the images of the training set batch by batch. #num_workers= no. of parallel threads
...
class G(nn.Module): # We introduce a class to define the generator. # Module refers to the different layers and connections in a neural network
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the generator. #self denotes the future object that will be created
        ...
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
            ...
            nn.ConvTranspose2d(512, 256, 4, 2, 1, bias = False), # We add another inversed convolution. # we keep dividing the input by 2 to get the output
            ...
            nn.ReLU(True), # We apply another ReLU.
            nn.ConvTranspose2d(64, 3, 4, 2, 1, bias = False), # We add another inversed convolution. # the output will be 3 because we want exactly 3 channels
            nn.Tanh() # We apply a Tanh rectification to break the linearity and stay between -1 and +1.
        )

    def forward(self, input): # We define the forward function that takes as argument an input that will be fed to the neural network, and that will return the output containing the generated images. # input  will be a random vector of  size of 100
...
class D(nn.Module): # We introduce a class to define the discriminator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the discriminator.
        super(D, self).__init__() # We inherit from the nn.Module tools. # We activate the inheritance using  the super function
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
...
# The error is between the prediction of the discriminator and the target ... but we will back propogate the error back inside the neural network of the generator
# Then we will update the weights by stochastic gradient descent
# BCE loss is  the binary cross entropy loss
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        target = Variable(torch.ones(input.size()[0])) # We get the target. # We use ones because  we want the accepted images. # If we use zeros we obtain the rejected images # Index "0" gives the actual size of the input
        ...
        noise = Variable(torch.randn(input.size()[0], 100, 1, 1)) # We make a random input vector (noise) of the generator. # 100 ,1,1 :- instead of having 100 values in the vector .. we will have 100 feature maps of size 1*1
        ...
        target = Variable(torch.zeros(input.size()[0])) # We get the target. # We input with zeros so that we can train for negative images
        output = netD(fake.detach()) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1). # We detached the gradients
        ...
        output = netD(fake) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1). #We want to keep the gradient of fake so we don't use fake.detach()
